...
func (tx *Tx) init() {
	tx.CallgraphNode = NewCallgraphNodeClient(tx.config)
	tx.CodeFile = NewCodeFileClient(tx.config)
	tx.UsageEvidence = NewUsageEvidenceClient(tx.config)
}
...
// applies a query, for example: CallgraphNode.QueryXXX(), the query will be executed
