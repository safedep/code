...
func (_q *ProjectQuery) QueryFiles() *FileQuery {
...
func (_q *ProjectQuery) WithFiles(opts ...func(*FileQuery)) *ProjectQuery {
	query := (&FileClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withFiles; query != nil {
		if err := _q.loadFiles(ctx, query, nodes,
			func(n *Project) { n.Edges.Files = []*File{} },
			func(n *Project, e *File) { n.Edges.Files = append(n.Edges.Files, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProjectQuery) loadFiles(ctx context.Context, query *FileQuery, nodes []*Project, init func(*Project), assign func(*Project, *File)) error {
	...
	query.Where(predicate.File(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.FilesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_files
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_files" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_files" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
