...
func (tx *Tx) init() {
	tx.ASTNode = NewASTNodeClient(tx.config)
	tx.CallRelationship = NewCallRelationshipClient(tx.config)
	tx.File = NewFileClient(tx.config)
	tx.ImportStatement = NewImportStatementClient(tx.config)
	tx.InheritanceRelationship = NewInheritanceRelationshipClient(tx.config)
	tx.Project = NewProjectClient(tx.config)
	tx.Symbol = NewSymbolClient(tx.config)
	tx.SymbolReference = NewSymbolReferenceClient(tx.config)
}
...
// applies a query, for example: ASTNode.QueryXXX(), the query will be executed
